...
def randomize_camera(initial_pos):
    elevation = random.uniform(0., 90.)
    azimuth = random.uniform(0., 360)
    distance = random.uniform(0.8, 1.6)
    return set_camera_location(elevation, azimuth, distance, initial_pos)

def set_camera_location(elevation, azimuth, distance, initial_pos=False):
...
def save_images(object_file: str) -> None:
    ...
    for i in range(args.num_images):
        ...
        camera = randomize_camera(initial_pos=(i == 0))
