...
def sample(
    ...
    if version == "svd":
        ...
        num_frames = 21
        num_steps = default(num_steps, 50)
        output_folder = default(output_folder, "outputs/simple_3d_sample/sv3d_c/")
        model_config = "scripts/sampling/configs/sv3d_c.yaml"
        cond_aug = 1e-5
        if isinstance(elevations_deg, float) or isinstance(elevations_deg, int):
            import random
            elevations_deg = [elevations_deg] * num_frames
...
def get_batch(keys, value_dict, N, T, device):
    ...
    for key in keys:
        ...
        elif key == "polars_rad" or key == "azimuths_rad" or key=="height_z":
